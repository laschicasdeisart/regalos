...
// Grupo "La Cápsula Creativa - Leads" en MailerLite. No es un dato sensible
// (a diferencia de la API key), así que va fijo aquí en vez de en una
// variable de entorno.
const GROUP_ID = "198762372956947915";

// Orígenes desde los que puede llamarse esta función (la página vive en
// laschicasdeisart.com/recursos, servida por GitHub Pages, fuera de Netlify).
var ALLOWED_ORIGINS = [
...
exports.handler = async function (event) {
  ...
  try {
    ...
    if (!response.ok) {
      console.log("subscribe-recursos: mailerlite rejected status=" + response.status + " body=" + responseBody);
      return {
        statusCode: 502,
        headers: headers,
    ...
    console.log("subscribe-recursos: exception " + String(err));
    return {
      statusCode: 502,
      headers: headers,
